...
class Commands(ABC):
    ...
    def font(self, font: Font, *, width: int | None = None) -> T:
        """
        Select a printer built-in font in a way that matches ESC/P semantics.

        Draft fonts are selected by print quality (ESC x), not by ESC k.
        LQ fonts are selected by leaving draft mode and then sending ESC k.
        """

        if font == Font.DRAFT:
            self.draft(False)
            self.condensed(False)
        elif font == Font.DRAFT_CONDENSED:
            self.draft(False)
            self.condensed(True)
        elif font == Font.ROMAN:
            self.draft(True)
            self.typeface(Typeface.ROMAN)
            self.condensed(False)
        elif font == Font.SANS_SERIF:
            self.draft(True)
            self.typeface(Typeface.SANS_SERIF)
            self.condensed(False)
        else:
